...
const AnalyticsPage = () => {
  ...
  if (!analytics) {
    return <section className="rounded-lg border border-slate-200 bg-white p-6 shadow-sm">Loading analytics...</section>;
  }
  ...
  return (
    <section className="space-y-5">
      <div>
        ...
        <p className="mt-1 text-sm text-slate-600">Revenue, order volume, and catalog performance in one view.</p>
      </div>
          ...
          <div className="mt-4 flex h-72 items-end gap-3">
            {analytics.salesTrend.map((point) => (
              <div key={point.month} className="flex h-full min-w-0 flex-1 flex-col justify-end gap-2">
                <div className="flex min-h-0 flex-1 items-end rounded-lg bg-slate-50 px-2">
                  <div
                ...
                <span className="text-center text-xs font-medium text-slate-500">{point.month}</span>
              </div>
            ))}
          </div>
        </article>
            ...
            {analytics.topProducts.map((product) => (
              <div key={product.id}>
                <div className="flex items-center justify-between text-sm">
                  ...
                  <span className="text-slate-500">{product.sales}</span>
                </div>
